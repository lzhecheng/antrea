...
func TestAddCipherSuitesToConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{&rawConfig, []uint16{0, 1}, []byte("ca-data"), "serverName", &expected},
	}
	for _, tc := range tests {
		origin := rest.CopyConfig(tc.rawConfig)
		err := AddCipherSuitesToConfig(tc.rawConfig, tc.cipherSuites, tc.caBundle, tc.serverName)
		if err != nil {
			t.Errorf("error: %v", err)
		} else {
			assert.Equal(t, *origin, *tc.rawConfig)
		}
	}
}
