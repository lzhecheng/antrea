...
func (c *client) serviceSNATFlows(nodeIP net.IP, localGatewayIP net.IP, localGatewayMAC net.HardwareAddr, category cookie.Category) []binding.Flow {
	...
	if c.enableProxy {
		flows = append(flows, []binding.Flow{
			// If the host uses another IP to access the Service, hostnetwork ClusterIP Service reply packet
			// needs unSNAT with snatCTMark mark.
			...
			c.pipeline[l3ForwardingTable].BuildFlow(priorityLow).MatchProtocol(binding.ProtocolIP).
				MatchInPort(config.HostGatewayOFPort).MatchCTMark(ServiceCTMark, nil).
