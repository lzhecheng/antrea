...
func NewAntreaClientProvider(config config.ClientConnectionConfiguration, kubeClient kubernetes.Interface, cipherSuites []uint16) *antreaClientProvider {
...
func (p *antreaClientProvider) updateAntreaClient() error {
	...
	if len(p.config.Kubeconfig) == 0 {
		...
		kubeConfig, err = inClusterConfig(caBundle, p.cipherSuites)
	} else {
		kubeConfig, err = clientcmd.NewNonInteractiveDeferredLoadingClientConfig(
			&clientcmd.ClientConfigLoadingRules{ExplicitPath: p.config.Kubeconfig},
			&clientcmd.ConfigOverrides{}).ClientConfig()
		kubeConfig.Transport = cipher.HttpTransportFromCipherSuites(p.cipherSuites)
	}
	if err != nil {
		return err
	}
...
func inClusterConfig(caBundle []byte, cipherSuites []uint16) (*rest.Config, error) {
	...
	if err := cipher.AddCipherSuitesToConfig(&config, cipherSuites, caBundle, cert.GetAntreaServerNames()[0]); err != nil {
		return nil, err
	}
	return &config, nil
}
