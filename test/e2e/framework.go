...
func (data *TestData) deployAntreaFlowExporter(ipfixCollector string) error {
	...
	ap := []configPair{
		{"FlowExporter", "true"},
		{"flowPollInterval", "\"1s\""},
		{"flowExportFrequency", "2"},
	}
	if ipfixCollector != "" {
		ap = append(ap, configPair{"flowCollectorAddr", fmt.Sprintf("flowCollectorAddr: \"%s\"", ipfixCollector)})
	}
	if testOptions.providerName == "kind" {
		...
		ap = append(ap, configPair{"enableTLSToFlowAggregator", "false"})
	}
	return data.mutateAntreaConfigMap([]configPair{}, ap, false, true)
}
...
func (data *TestData) mutateAntreaConfigMap(controllerPair []configPair, agentPair []configPair, restartController, restartAgent bool) error {
	configMap, err := data.GetAntreaConfigMap(antreaNamespace)
	if err != nil {
		return err
	}
	...
	for _, p := range controllerPair {
		controllerConf = replaceFieldValue(controllerConf, p.field, p.value)
	}
	configMap.Data["antrea-controller.conf"] = controllerConf
	agentConf, _ := configMap.Data["antrea-agent.conf"]
	for _, p := range agentPair {
		agentConf = replaceFieldValue(agentConf, p.field, p.value)
	}
...
func replaceFieldValue(content string, field string, value string) string {
	r := regexp.MustCompile(fmt.Sprintf(`(?m)#?.*%s:.*$`, field))
	res := r.ReplaceAllString(content, fmt.Sprintf("%s: %s", field, value))
	return res
}
