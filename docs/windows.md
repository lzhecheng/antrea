...
mv antrea-agent-windows-x86_64.exe "${KubernetesHome}/antrea/bin/antrea-agent.exe"
...
with docker due to a bug in the way Containerd attaches host networks for Windows Pods. In any case, if you are using
docker on Windows, this is how you can run Antrea in a Pod.
